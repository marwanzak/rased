$(document).ready(
		function() {
			...
			$(".btnc").on(
					"click",
					function() {
						if ($(this).parent().find("input[type=checkbox]").is(
								":checked")) {
							$(this).parent().find("input[type=checkbox]").prop(
									"checked", false);
							$(this).val("تم حلها");
						} else {
							$(this).parent().find("input[type=checkbox]").prop(
		...
		async : false
			...
			function(data) {
				...
				$('<option/>').val('').html('اختر نوع الملاحظة').appendTo(
						probselect);
				for ( var i = 0; i < data.length; i++) {
					$('<option/>').val(data[i].id).html(data[i].prob).css("background-color",data[i].color).appendTo(
							probselect);
				}
			});
	return false;
