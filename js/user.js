		...
		function() {
			...
			$(".add_user_student_but").on("click", function() {
				$.ajax({
					...
					data : {
						"idnum" : $("#new_user_student_text").val()
					},
					type : "post",
					success : function(data) {
						switch (data) {
			...
			$("#check_idnum_but").on("click", function() {
				$.ajax({
					url : base_url + "user/checkIdnumExist",
					data : {
						"idnum" : $("#add_student_idnum_text").val()
					},
					type : "post"
				}).done(function(data) {
					alert(data);
			...
			$("#show_lessons_form").on("submit", function() {
				if ($("#lessons_class_select").val() == "") {
					alert("يجب اختيار فصل");
					return false;
				}
			});
		});
...
function sliderArray() {
	var slides = $(".user-slider div").length;
	$(".user-slider > div:gt(0)").hide();
	setInterval(function() {
		$('.user-slider > div:first').fadeOut(2000).next().fadeIn(2000).end()
				.appendTo('.user-slider');
	}, 2000 * slides);
}